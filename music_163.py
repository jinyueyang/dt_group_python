# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 11:17:03 2017
...
def catchSongs(url_id,url):
    ...
    driver = webdriver.PhantomJS(executable_path=r'C:\phantomjs\phantomjs\bin\phantomjs.exe')#,executable_path='/Users/mrlevo/phantomjs-2.1.1-macosx/bin/phantomjs')  # 注意填上路径
    driver.get(url)
        ...
        try:
            ...
            while wrong_time < 5:  # 不断获取歌信息，假定5次获取不到值，就判无值可获取，跳出循环
                try:
                    ...
                    write2txt(new_line,user)  # mac写入文件需要改变字符，以id命名的文件，存储在执行脚本的当前路径下，在win下请去掉编.endcode('utf-8')
...
if __name__ == '__main__':
    ...
    for url in ['http://music.163.com/user/home?id=119022303']:  # 这里把自己的id替换掉，想爬谁的歌单都可以，只要你有他的id
